...
public class Computer extends Game {
    ...
    public boolean isSafe(int x, int y) {
        ...
        return x >= 0 & x < board.length && y >= 0 && y < board[0].length && (board[x][y]=='x' | board[x][y]=='-');
    }
    ...
    public void placeMarkByComputer() {
        ...
        if (rowIndex!=null) {
            ...
            getBoard()[row][col] = getCurrentPlayer();
    ...
    private boolean[][] findAvailableSpots() {
        if(!isBoardFull()) {
            boolean[][] spot = new boolean[board.length][board[0].length];
